...
genomes<-readLines(here("..","input","primer_files",paste0(primer_version,".fasta")))
header<-genomes[grep(">",genomes)]
genomes <- grep("^>", genomes, invert = TRUE, value = TRUE)
genomes<-paste(genomes, collapse = "")
genome_sequences_df<-data.frame(genome_name=header,sequence=genomes)
...
for(sample in samples){
  ...
  mastersheet_temp <- mastersheet[order(mastersheet[,1]), ]
  ...
  virus_plots<-list()
  i<-0
  giant_df<-data.frame(start=c(1:50000))
  for(virus in viruses){
    ...
    for(variant in c(1:nrow(variant_temp))){
      ...
      variant_pos_genome<-min(mastersheet_temp$amp_start[mastersheet_temp$wfamplicon_name==amplicon])+variant_temp$Position[variant]  
    ...
    virus_sequence<-genome_sequences_df$sequence[1]
    ...
    for(amplicon in amplicons){ #get depth
      ...
      depth_temp$start<-depth_temp$start+min(mastersheet_temp$amp_start[mastersheet_temp$wfamplicon_name==amplicon])
      depth_temp$end<-depth_temp$end+min(mastersheet_temp$amp_start[mastersheet_temp$wfamplicon_name==amplicon])
    ...
    for (row in 1:nrow(current_amplicons)) {
      ...
      r_len <- nchar(current_amplicons$revcomp_primer[row])
...
for(amplicon in c(1:nrow(mastersheet_temp))){
  ...
  revP_end<-as.numeric(mastersheet_temp$amp_end[amplicon]+nchar(mastersheet_temp$revcomp_primer[amplicon]))
...
amplicon_region<-paste0("{
            ...
            },
       {
                ...
                xref: 'x',
                yref: 'paper',
                x0: ",amplicon_end,",            // rev primer Start (bp)
                x1: ",revP_end,",            // rev primer End (bp)
                y0: ",(y_pos/max_y),",           // Height start (1% from bottom of grid)
                y1: ",(y_pos/max_y)+0.01,",           // Height stop (4% from bottom of grid)
